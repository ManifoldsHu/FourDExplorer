...
def RollingDiffractionPattern(
    ...
    if not isinstance(data_object, (h5py.Dataset, np.ndarray)):
        raise TypeError(
            "data object must be a np.ndarray or h5py.Dataset, not {0}".format(
                type(data_object).__name__
            )
        )
    if not isinstance(result_object, (h5py.Dataset, np.ndarray)):
        raise TypeError(
            "result object must be a np.ndarray or h5py.Dataset, not {0}".format(
    ...
    if result_object.shape != data_object.shape:
        raise ValueError(
            "result object's shape must be the same as the source data object's shape."
        )
        ...
        for jj in range(scan_j):
            with result_lock:
                result_object[ii, jj, :, :] = np.roll(
                    data_object[ii, jj, :, :], translation_vector, axis=(0, 1)
                )
...
def TranslatingDiffractionPattern(
    ...
    if shift_mapping.shape[1:2] != data_object.shape[:1]:
        raise ValueError(
            f"shape of shift_mapping {shift_mapping.shape} does not match the scanning shape of 4D-STEM dataset {data_object.shape}"
        )
