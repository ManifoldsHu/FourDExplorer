...
class ImporterEMPAD(QObject):
    ...
    def _parseLoadingData(self, root: Document):
        ...
        if root.getElementsByTagName("pix_x"):
            ...
            for mode in root.getElementsByTagName("scan_parameters"):
                if mode.getAttribute("mode") == "acquire":
                    self.scan_j = int(self._getData(mode, "scan_resolution_x"))
                    self.meta["/Calibration/Space/scan_j"] = self.scan_j
        ...
        if root.getElementsByTagName("pix_y"):
            ...
            for mode in root.getElementsByTagName("scan_parameters"):
                if mode.getAttribute("mode") == "acquire":
                    self.scan_i = int(self._getData(mode, "scan_resolution_y"))
                    self.meta["/Calibration/Space/scan_i"] = self.scan_i
    ...
    def _parseCalibrateData(self, root: Document):
        ...
        try:
            ...
            self.logger.error(
                "Failed to parse scan_size item.\n{0}".format(e), exc_info=True
            )
        ...
        try:
            ...
            step_size_i = (
                self.full_scan_field_of_view_i
                * self.scale_factor
                * self.scan_size
                / self.scan_i
            )
            step_size_j = (
                self.full_scan_field_of_view_j
                * self.scale_factor
                * self.scan_size
                / self.scan_j
            )
            ...
            self.logger.error(
                "Failed to parse scanning_step_size items.\n{0}".format(e),
                exc_info=True,
            )
        ...
        try:
            ...
            self.logger.error(
                "Failed to parse camera_length item.\n{0}".format(e), exc_info=True
            )
        ...
        try:
            ...
            self.logger.error(
                "Failed to parse scan_rotation item.\n{0}".format(e), exc_info=True
            )
    ...
    def _parseOtherData(self, root: Document):
        ...
        try:
            ...
            self.meta["/Acquisition/Microscope/acquisition_time"] = (
                parsed_datetime.strftime("%H:%M:%S")
            )
            self.meta["/Acquisition/Microscope/acquisition_date"] = (
                parsed_datetime.strftime("%Y-%m-%d")
            )
        except Exception as e:
            self.logger.error(
                "Failed to parse acquire_datetime item.\n{0}".format(e), exc_info=True
            )
        ...
        try:
            ...
            self.logger.error(
                "Failed to parse empad_version item.\n{0}".format(e), exc_info=True
            )
...
class ImporterEMPAD_NJU(ImporterEMPAD):
    ...
    def _parseCalibrateData(self, root: Document):
        ...
        try:
            ...
            self.meta["/Acquisition/Microscope/step_size_i"] = self.meta[
                "/Calibration/Space/scan_dr_i"
            ]
            self.meta["/Acquisition/Microscope/step_size_j"] = self.meta[
        ...
        try:
            ...
            self.logger.error(
                "Failed to parse screen_current item.\n{0}".format(e), exc_info=True
            )

        try:
            iom = root.getElementsByTagName("iom_measurements")[0]
            du_in_rad = float(self._getData(iom, "calibrated_diffraction_angle"))
            ...
            self.meta["/Calibration/Space/dr_i"] = (
                self.dp_i / self.meta["/Calibration/Space/du_i"]
            )
            self.meta["/Calibration/Space/dr_j"] = (
                self.dp_j / self.meta["/Calibration/Space/du_j"]
            )
        except Exception as e:
            self.logger.error(
                "Failed to parse reciprocal_pixel_size item.\n{0}".format(e),
                exc_info=True,
            )
