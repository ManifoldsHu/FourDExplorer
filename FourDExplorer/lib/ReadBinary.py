...
def getDType(
    ...
    if scalar_type not in scltp:
        raise TypeError("scalar_type must be one of {0}".format([key for key in scltp]))
    if little_endian:
...
def readFourDSTEMFromDM4(
    ...
    with open(file_path, "rb") as fid:
        ...
        data = np.nan_to_num(
            np.fromfile(fid, dtype=dt, count=dp_i * dp_j * scan_i * scan_j)
        )  # convert to chunk loading
        dataset[:, :, :, :] = data.reshape((scan_i, scan_j, dp_i, dp_j))


def readDataFromHDF5(
    file_path: str, dataset_path: str, item_path: str, progress_signal: Signal = None
):
    """
