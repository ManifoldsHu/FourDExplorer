...
def loadFDDNetModel() -> ort.InferenceSession:
    ...
    with importlib.resources.path("models", "FDDNet.onnx") as model_path:
        ...
        session_fddnet_options.graph_optimization_level = (
...
def loadFDDNetAngleModel() -> ort.InferenceSession:
    ...
    with importlib.resources.path("models", "FDDNetAngle.onnx") as model_path:
        ...
        session_fddnet_angle = ort.InferenceSession(
            str(model_path), session_fddnet_options
        )
...
def inferEllipseLoc(image: np.ndarray, ort_session: ort.InferenceSession):
    ...
    outputs = ort_session.run(
        None, {input_name: image[np.newaxis, np.newaxis, :, :].astype(np.float32)}
    )[0][0]
...
def _phaseShiftDecoder(x_n: np.ndarray, mapping_cycle: int = 2) -> float:
    ...
    phi = -np.arctan2(
        np.sum(x_n * np.sin(phase_shift)), np.sum(x_n * np.cos(phase_shift))
    )
...
def inferEllipse(
    image: np.ndarray,
    ort_session_fddnet: ort.InferenceSession,
    ort_session_fddnetangle: ort.InferenceSession,
):
    """
    ...
    outputs = ort_session_fddnet.run(
        None,
        {input_name_fddnet: image[np.newaxis, np.newaxis, :, :].astype(np.float32)},
    )[0][0]
    pred_ci, pred_cj, pred_a, pred_b = outputs
    ...
    pred_xn = ort_session_fddnetangle.run(
        None,
        {
            input_name_fddnet_angle: image[np.newaxis, np.newaxis, :, :].astype(
                np.float32
            )
        },
    )[0][0]
...
def mapInferenceFDDNetEllipse(
    ...
    result_lock = Lock()
    ci, cj, a, b, angle = [
        np.zeros((scan_i, scan_j), dtype="float32") for _ in range(5)
    ]
    fddnet_model = loadFDDNetModel()
    for ii in range(scan_i):
        for jj in range(scan_j):
            dp = np.asarray(dataset[ii, jj, :, :])
            with result_lock:
                ci[ii, jj], cj[ii, jj], a[ii, jj], b[ii, jj], angle[ii, jj] = (
                    inferEllipse(fddnet_model)
                )
        progress_signal.emit(int((ii + 1) / scan_i * 100))
    return angle
