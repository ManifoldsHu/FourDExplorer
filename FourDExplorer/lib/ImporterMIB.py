    ...
    def parseHdrFile(self, hdr_path: str):
        ...
        with open(hdr_path, encoding="utf-8") as hdr_file:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to parse Frames per Trigger in .hdr file: {e}  ",
                    exc_info=True,
                )
            ...
            try:
                self._scan_i = (
                    int(_hdr_meta["Frames in Acquisition (Number)"]) // self._scan_j
                )
                self.meta["/Calibration/Space/scan_i"] = self._scan_i
                self._scan_i_in_hdr = True
            except Exception as e:
                self.logger.error(
                    f"Failed to parse Frames in Acquisition in .hdr file: {e}",
                    exc_info=True,
                )
