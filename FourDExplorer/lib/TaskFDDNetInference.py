...
class TaskFDDNetInference(Task):
    ...
    def __init__(
        ...
        if not set(calc_dict.keys()).issubset(self._available_modes):
            raise ValueError(
                f"calc_dict keys must be a subset of {self._available_modes}"
            )
        ...
        for key in self._calc_dict:
            if self._calc_dict[key] and key not in names_dict:
                raise ValueError(
                    f"The key '{key}' in calc_dict is True but not found in names_dict."
                )
        ...
        for key in self._calc_dict:
            if self._calc_dict[key] and key not in metas_dict:
                raise ValueError(
                    f"The key '{key}' in calc_dict is True but not found in metas_dict."
                )
    ...
    def _workerFDDNetInference(self, progress_signal: Signal = None):
        ...
        if _is_angle and _is_scale:
            ci, cj, a, b, angle = mapInferenceFDDNetEllipse(
                self.stem_path, progress_signal
            )
        elif _is_angle:
                    ...
                    if mode in ["ci", "cj", "a", "b", "angle"]:
                        ...
                        if params is not None:
                            if fit_model_name == "Linear":
                                result = linearModel(locations.T, *params).reshape(
                                    scan_i, scan_j
                                )
                            elif fit_model_name == "Quadratic":
                                result = quadraticModel(locations.T, *params).reshape(
                                    scan_i, scan_j
                                )
                    elif mode == "center":
                        ...
                        if ci_params is not None and cj_params is not None:
                            if fit_model_name == "Linear":
                                ci_result = linearModel(
                                    locations.T, *ci_params
                                ).reshape(scan_i, scan_j)
                                cj_result = linearModel(
                                    locations.T, *cj_params
                                ).reshape(scan_i, scan_j)
                            elif fit_model_name == "Quadratic":
                                ci_result = quadraticModel(
                                    locations.T, *ci_params
                                ).reshape(scan_i, scan_j)
                                cj_result = quadraticModel(
                                    locations.T, *cj_params
                                ).reshape(scan_i, scan_j)
    ...
    def _fitModel(
        self, model_name: str, locations: np.ndarray, values: np.ndarray
    ) -> np.ndarray | None:
        """
