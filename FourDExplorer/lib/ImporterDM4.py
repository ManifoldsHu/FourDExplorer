...
class TagDirectory(TagObject):
    ...
    def show_tags(self, level=0, show_max_level=0):
        ...
        for i in range(self.num_tags):
            ...
            print(
                f"{'    ' * level}{self.name} ({i + 1}/{self.num_tags}), {current_tag.get_name()}, type: {current_tag.get_type()}"
            )
    ...
    def read(self):
        with open(self.path, "rb") as f:
            ...
            for i in range(self.number_of_entries):
                data = struct.unpack(self.byte_order + self.dtype, f.read(self.dsize))[
                    0
                ]
...
tagDataType = {
    ...
    0x04: {
        "format": "H",
        "description": "unsigned short (i2*) or unicode string",
        "size": 2,
    },
    ...
    0x14: {
        "format": "array",
        "description": "array of data/array of group",
        "size": None,
    },  # Size varies
}
...
class ParseDM4(QObject):
    def __init__(self, file_path, parent=None):
        ...
        self.byte_order = (
            ">"  # Default to big-endian ('>' for big-endian, '<' for little-endian)
        )
    ...
    def read_header(self):
        ...
        self.logger.debug(
            f"Header read successfully, DM version: {dm_version}, Root length: {root_len}, Byte order: {self.byte_order}."
        )

    def read_tag_directory(
        self, parent_dir_name=None, tag_dir_name=None, tag_dir_len=None
    ):
        f = self.io
    ...
    def parse_tag(self, parent_dir_name=None):
        ...
        if tag_type == 20:  # Tag Directory
            ...
            tag_object = self.read_tag_directory(
                parent_dir_name=parent_dir_name,
                tag_dir_name=tag_name,
                tag_dir_len=tag_len,
            )
        elif tag_type == 21:  # Tag
            tag_object = self.read_tag(
                parent_dir_name=parent_dir_name, tag_name=tag_name
            )
        ...
        if tag_info_len == 1:  # Single data entry
            ...
            if tag_info[0] == 15:  # Group of data i.e. struct
                ...
                for i in range(num_entries):
                    ...
                    data = struct.unpack(
                        self.byte_order + data_type, f.read(data_size)
                    )[0]
                    data_entries.append(data)
    ...
    def parse(self):
        ...
        dm4obj = self.read_tag_directory(
            parent_dir_name="", tag_dir_name="/", tag_dir_len=self.byte_size
        )
    ...
    def parseDM4(self, dm4_path: str):
        ...
        if len(taglist) > 1:
            ...
            for i in range(0, len(taglist)):
                ...
                self.logger.debug(
                    f"Checking the {i + 1}th 4D-STEM dataset, scalar type: {scalar_type}."
                )
                if (
                    scalar_type == "uint"
                    or scalar_type == "int"
                    or scalar_type == "float"
                ):
                    ind = i
        ...
        self.logger.debug(
            f"4D-STEM scalar type: {tagdir_4dstem.get_tag_by_name('Data').data.type}, scalar size: {tagdir_4dstem.get_tag_by_name('Data').data.dsize}."
        )
        ...
        camera_length = microscope_info.get_tag_by_name(
            "STEM Camera Length"
        ).get_data()  # mm

        digiscan = tagdir_4dstem.get_tag_by_name("DigiScan")
        horizontal_spacing = digiscan.get_tag_by_name(
            "Horizontal Spacing"
        ).get_data()  # nm ?
        vertical_spacing = digiscan.get_tag_by_name(
            "Vertical Spacing"
        ).get_data()  # nm ?
        ...
        self.meta["/Calibration/Space/du_i"] = (
            camera_pixel_size[0] * 1e-6 / (camera_length * 1e-3 * wavelength)
        )
        self.meta["/Calibration/Space/du_j"] = (
            camera_pixel_size[1] * 1e-6 / (camera_length * 1e-3 * wavelength)
        )
