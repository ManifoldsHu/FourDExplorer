    ...
    def __setitem__(self, key: str, value: list[float]):
        ...
        if key in self._aberration_angle_multipliers:
            if isinstance(value, complex):
                ...
                self._mapping[key] = self._calc_complex_aberration(
                    key, value[0], value[1]
                )
            else:
                raise TypeError(
...
class OpticalSTEM(object):
    ...
    def calcWaveLength(accelerate_voltage: float):
        ...
        return (
            h
            * c
            / np.sqrt((e * accelerate_voltage + 2 * m * c**2) * e * accelerate_voltage)
        )
    ...
    def _adjust_bright_field_disk_radius(self):
        ...
        """
        self._bright_field_disk_radius = (
            self._alpha * self._camera_length / self._detector_pixel_size
        )
    ...
    def _adjust_camera_length(self):
        ...
        """
        self._camera_length = (
            self._bright_field_disk_radius * self._detector_pixel_size / self._alpha
        )
    ...
    def setAberrationDict(
        self, aberration_dict: AberrationDict, update_cache: bool = True
    ):
        """
        设置此光路的各项像差
    ...
    def setDetectorPixelSize(
        self, detector_pixel_size: float, update_cache: bool = True
    ):
        """
        设置相机像素在物理上的尺寸。
    ...
    def setBrightFieldDiskRadius(
        self, bright_field_disk_radius: float, update_cache: bool = True
    ):
        """
    ...
    def setAberrationCoefficient(
        self, key: str, value: float | complex | list[float], update_cache: bool = True
    ):
        """
        设置某项像差系数。
    ...
    def setAberrationCoefficientModule(
        self, key: str, value: float, update_cache: bool = True
    ):
        """
        设置某项像差系数的模长。
    ...
    def setAberrationCoefficientAngle(
        self, key: str, value: float, update_cache: bool = True
    ):
        """
    ...
    def generateVirtualAnnularDetector(
        self, beta_min: float, beta_max: float
    ) -> np.ndarray:
        """
    ...
    def generateScanMeshgrid(self) -> tuple[np.ndarray]:
        ...
        """
        scan_rii = self.dx * np.linspace(
            -self.scan_N / 2, self.scan_N / 2 - 1, self.scan_N
        )
        scan_rjj = self.dx * np.linspace(
            -self.scan_N / 2, self.scan_N / 2 - 1, self.scan_N
        )
    ...
    def generateProbePositions(self, scan_angle) -> np.ndarray:
        ...
        for i in range(_scan_i):
            for j in range(_scan_j):
                probe_positions[i, j, 0] = (i + 1) * scan_step_size / dx * np.cos(
                    scan_angle
                ) - (j + 1) * scan_step_size / dx * np.sin(scan_angle)
                probe_positions[i, j, 1] = (i + 1) * scan_step_size / dx * np.sin(
                    scan_angle
                ) + (j + 1) * scan_step_size / dx * np.cos(scan_angle)
    ...
    def generatePaddedProbePositions(self, scan_angle, padding: int = 50) -> np.ndarray:
        ...
        padded_probe_positions[:, :, 0] = (
            probe_positions[:, :, 0]
            - np.floor(np.min(probe_positions[:, :, 0]))
            + padding
        )
        padded_probe_positions[:, :, 1] = (
            probe_positions[:, :, 1]
            - np.floor(np.min(probe_positions[:, :, 1]))
            + padding
        )
    ...
    def generatePaddedObject(self, scan_angle, padding: int = 50) -> np.ndarray:
        ...
        padded_object_shape = (
            np.max(padded_probe_positions[:, :, 0])
            - np.min(padded_probe_positions[:, :, 0])
            + self._dp_i
            + 2 * padding,
            np.max(padded_probe_positions[:, :, 1])
            - np.min(padded_probe_positions[:, :, 1])
            + self._dp_j
            + 2 * padding,
        )
    ...
    def generateRonchigram(self) -> np.ndarray:
        ...
        transmission = (
            np.ones((dp_N, dp_N)) + np.random.rand(dp_N, dp_N) * 2 * np.pi * 1j
        )
...
class CTFCalculator(object):
    ...
    def calcCTFofVirtualImageFirstOrder(
        self, beta_min: float, beta_max: float, update_cache: bool = True
    ):
        """
    ...
    def calcCTFofVirtualImageSecondOrder(
        self, beta_min: float, beta_max: float, rho=1e-10, update_cache: bool = True
    ):
        """
        ...
        G_D = bessel_j0(2 * np.pi / wave_length * beta_min * rho) - bessel_j0(
            2 * np.pi / wave_length * beta_max * rho
        )
    ...
    def calcCTFofAxialBF(self, update_cache=True):
        ...
        return (
            np.real(ctfab)
            * np.pi
            * (self.optical_stem.alpha / self.optical_stem.wave_length) ** 2
        )
    ...
    def generateRadialPreimage(
        ...
        distance_distribution = np.round(
            np.sqrt((i_index - target[0]) ** 2 + (j_index - target[0]) ** 2)
        )
        ...
        for ii in range(matrix_shape[0]):
            for jj in range(matrix_shape[1]):
                locate[int(distance_distribution[ii, jj] - min_distance)].append(
                    (ii, jj)
                )
    ...
    def generateRotationalAverage(
        ...
        if preimage_locate is None:
            ...
            if preimage_distance is None:
                raise RuntimeError(
                    "Must pass preimage_distance parameter when using preimage_locate"
                )
        ...
        for ii in range(len(distance)):
            radial_distribution[ii] = np.mean(
                distribution_array[locate[ii][:, 0] * shape[1] + locate[ii][:, 1]]
            )
        return distance, radial_distribution

    def calcCTFofTEM(
        self, objective_aperture_radius: float = None, chromatic_delta: float = 0
    ):
        """
        ...
        if chromatic_delta != 0:
            ...
            Ecexp = np.exp(
                -1
                / 2
                * (np.pi * self.optical_stem.wave_length * chromatic_delta) ** 2
                * (ui**2 + uj**2) ** 2
            )
        else:
            Ecexp = np.ones(
                (self.optical_stem.getDetectorN(), self.optical_stem.getDetectorN())
            )
        if objective_aperture_radius is not None:
            aperture = self.optical_stem.generateVirtualAnnularDetector(
                beta_min=0, beta_max=objective_aperture_radius
            )
        else:
            aperture = np.ones(
                (self.optical_stem.getDetectorN(), self.optical_stem.getDetectorN())
