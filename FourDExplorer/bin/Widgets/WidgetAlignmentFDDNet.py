...
class WidgetAlignmentFDDNet(QWidget):
    ...
    def _initUi(self):
        ...
        self.ui.checkBox_show_measured_ellipse.stateChanged.connect(
            self._updateEllipsePatch
        )
        self.ui.pushButton_adjust_ellipse_effects.clicked.connect(
            self.adjustEllipseEffects
        )
    ...
    def inference(self) -> tuple[float]:
        ...
        ci, cj, a, b, theta = inferEllipse(
            dp, self._session_fddnet, self._session_fddnet_angle
        )
    ...
    def getCurrentDPShiftVec(self):
        ...
        """
        if (
            self.data_path == self._current_record["data_path"]
            and self.scan_ii == self._current_record["scan_ii"]
            and self.scan_jj == self._current_record["scan_jj"]
        ):
    ...
    def _updateEllipsePatch(self):
        ...
        self._ellipse_patch.set_center(
            (
                self._current_cj + center_j,
                self._current_ci + center_i,
            )
        )
    ...
    def generateShiftMapping(self):
        ...
        centers_meta = self._generateShiftMapMeta(
            name=shift_map_name, fit_model_name=fit_model
        )
        metas_dict = {"center": centers_meta}

        fit_models_dict = {"center": fit_model}
...
class DialogGenerateShiftVector(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.radio_button_apply_current_shift_vec = QRadioButton(
            "None (Do not use fit)"
        )
