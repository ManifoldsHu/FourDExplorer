...
class PageViewVectorField(QWidget):
    ...
    def setBackground(self, background_path: str):
        ...
        if not isinstance(background_path, str):
            raise TypeError(
                "background_path must be a str, not {0}".format(
                    type(background_path).__name__
                )
            )
        ...
        if not isinstance(background_node, HDFDataNode):
            raise ValueError("Item {0} must be a Dataset".format(background_path))
        ...
        _, height, width = self.data_object.shape
        if (
            background_data_obj.shape[0] != height
            or background_data_obj.shape[1] != width
        ):
            raise ValueError(
                "The background image's shape must be equal to"
    ...
    def _createColorbar(self):
        ...
        if self._colorbar_object is None:
            self._colorbar_object = Colorbar(
                ax=self.colorbar_ax, mappable=self.background_object
            )
        else:
    ...
    def _initUi(self):
        ...
        self.ui.checkBox_background_visible.stateChanged.connect(self._setImageVisible)

        self.ui.pushButton_browse.clicked.connect(self._browse)
        self.ui.pushButton_browse_background.clicked.connect(self._browseBackground)
        # self.ui.pushButton_vector_processing.clicked.connect(
        #     self._vectorProcessing
        # )
        self.ui.pushButton_adjust_effects.clicked.connect(self._adjustEffects)
...
class DialogAdjustQuiverEffect(QDialog):
    ...
    def setVectorField(self, item_path: str):
        ...
        self.ui.comboBox_color.setCurrentIndex(self._colors[self.data_color])
...
class DialogVectorProcessing(QDialog):
    ...
    def __init__(self, parent: QWidget):
        ...
        self.ui.pushButton_flip.clicked.connect(self._flip)
        self.ui.pushButton_calculate_curl.clicked.connect(self._calculate_curl)
        ...
        self.ui.pushButton_vec_i.clicked.connect(self._slice_vec_i)
        self.ui.pushButton_vec_j.clicked.connect(self._slice_vec_j)
