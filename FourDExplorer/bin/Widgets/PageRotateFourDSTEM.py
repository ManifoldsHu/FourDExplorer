...
class PageRotateFourDSTEM(PageBaseFourDSTEM):
    ...
    def _initUi(self):
        ...
        self.ui.pushButton_browse_vector_field.clicked.connect(self._browseVectorField)
        ...
        self.ui.toolButton_refresh_quiver.clicked.connect(self._changeQuiverAngle)
        ...
        self.ui.pushButton_calculate_rotation_angle_axial_bf.clicked.connect(
            self._calculateRotationAngleAxialBF
        )
        ...
        self.ui.toolButton_refresh_axial_bf.clicked.connect(self._updateDP)
    ...
    def setVectorField(self, vec_path: str = "", read_attr: bool = False):
        ...
        if not isinstance(vec_path, str):
            raise TypeError(
                "vec_path must be a str, not {0}".format(type(vec_path).__name__)
            )
        ...
        if not len(vec_obj.shape) == 3:
            raise ValueError("Data must be a 3D matrix with shape (2, i, j)")
    ...
    def _calculateRotationAngle(self):
        ...
        if self.vec_path == "":
            ...
            msg.setStandardButtons(QMessageBox.Ok)
            msg.setText(
                "Should set a Center of Mass vector field first.\n"
    ...
    def _generateMeta(self, name: str = None) -> dict:
        ...
        meta["/Calibration/RotationalOffsetCorrection/scan_rotation"] = (
            self.rotation_angle
        )
    ...
    def setAxialBF(self, axial_bf_path: str, read_attr: bool = False):
        ...
        if not isinstance(axial_bf_path, str):
            raise TypeError(
                f"axial_bf_path should be a string, not {type(axial_bf_path).__name__}"
            )
