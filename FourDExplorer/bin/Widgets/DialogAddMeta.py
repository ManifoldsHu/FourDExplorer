...
class DialogAddMeta(QDialog):
    ...
    def _slotOkClicked(self):
        ...
        if meta_key in self.meta_manager.listSchemaKeys():
            ...
            msg_box_ask.setText(
                f"Modifying this metadata may affect how the software works."
            )
        ...
        if meta_key in self.hdf_handler.file[item_path].attrs:
            ret = QMessageBox.information(
                self,
                "Info",
                f"There has been an existing metadata: <code>{meta_key}</code>. Do you want to overwrite it?",
    ...
    def readMetaFromFile(self):
        ...
        if not meta_key:
            return
        if meta_key in self.hdf_handler.file[item_path].attrs:
            self.logger.warning(f"Metadata key {meta_key} exists in {item_path}")
            QMessageBox.warning(
                self,
                "Warning",
    ...
    def _setExistMeta(self):
        ...
        if isinstance(meta_value, (int, np.integer)):
            ...
            QMessageBox.warning(
                self, "Warning", f"Unsupported data type: {type(meta_value)}"
            )
