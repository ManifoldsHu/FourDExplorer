...
class DialogEditParaFourDSTEM(QDialog):
    ...
    def _updateNavigationLabel(self):
        ...
        formatted_parts = [
            f"<b>{part}</b>" if i == current_index else part
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if len(dataset.shape) == 4:  # Check if the dataset is 4D
                ...
                QMessageBox.warning(
                    self,
                    "Invalid Dataset",
                    "The selected dataset is not a 4D-STEM dataset.",
                )
    ...
    def setFourDSTEM(self, dataset_path: str):
        ...
        """
        if (
            dataset_path not in self.hdf_handler.file
        ):  # Check if the dataset exists in the HDF5 file
            raise ValueError(
                f"Dataset does not exist in the HDF5 file: {dataset_path}."
            )
    ...
    def _setMetaValue(self, key: str, widget: QWidget):
        ...
        if display_unit and default_unit:
            meta_value = self.unit_manager.convert(
                meta_value, default_unit, display_unit
            )
    ...
    def _editMeta(self, key: str, display_widget: QWidget):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            self.logger.info(
                f"Edited metadata {key}: {self.meta_manager.getValue(key)}"
            )
    ...
    def _initPageSpace(self):
        ...
        for key, widgets in self.space_metadata_map.items():
            if widgets["edit"] is not None:
                widgets["edit"].clicked.connect(self._editMetaWrapper(key, widgets))
                widgets["display"].setToolTip(
                    self.meta_manager.getSchemaDescription(key)
                )
            self._setMetaValue(key, widgets["display"])
