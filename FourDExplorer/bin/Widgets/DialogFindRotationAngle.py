...
class DialogFindRotationAngle(QDialog):
    ...
    def _createAxes(self):
        ...
        if self._curve_ax is None:
            ...
            self.curve_ax.set_title(
                "Sum of point-wise squared distribution - angle curve"
            )
    ...
    def _calculateLines(self):
        ...
        for ii, angle_deg in enumerate(self._curve_angle):
            ...
            self._curve_div[ii] = np.sum(
                np.square(Divergence2D(vec_rotated[0, :, :], vec_rotated[1, :, :]))
            )

            self._curve_curl[ii] = np.sum(
                np.square(Curl2D(vec_rotated[0, :, :], vec_rotated[1, :, :]))
            )
