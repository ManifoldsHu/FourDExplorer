...
class DialogScaleBar(QDialog):
    ...
    def _initUi(self):
        ...
        self.ui.comboBox_real_length_unit.currentIndexChanged.connect(
            self.updateScaleBar
        )
        self.ui.comboBox_reciprocal_unit.currentIndexChanged.connect(
            self.updateScaleBar
        )
        self.ui.comboBox_text_bg_edge_color.currentIndexChanged.connect(
            self.updateScaleBar
        )
        self.ui.comboBox_text_bg_fill_color.currentIndexChanged.connect(
            self.updateScaleBar
        )
        self.ui.comboBox_text_color.currentIndexChanged.connect(self.updateScaleBar)
        self.ui.comboBox_text_font_family.currentIndexChanged.connect(
            self.updateScaleBar
        )
        self.ui.comboBox_text_font_style.currentIndexChanged.connect(
            self.updateScaleBar
        )
        ...
        self.ui.doubleSpinBox_text_bg_edge_width.valueChanged.connect(
            self.updateScaleBar
        )
        self.ui.doubleSpinBox_text_bg_fill_alpha.valueChanged.connect(
            self.updateScaleBar
        )
    ...
    def _updateScaleBarLength(self):
        ...
        if not self.ui.checkBox_customize_scale.isChecked():
            try:
                ...
                bar_length_pix = (
                    self.unit_manager.convert(
                        bar_length, display_unit, self.pixel_length_unit
                    )
                    / self.pixel_length
                )
            except ValueError as e:
    ...
    def _updateScaleBarTextStyle(self):
        ...
        self._scale_bar_text.set_color(self._getMatplotlibColor(text_color))
        ...
        self._scale_bar_text.set_fontsize(self.ui.spinBox_text_font_size.value())
        self._scale_bar_text.set_fontstyle(
            self.ui.comboBox_text_font_style.currentText()
        )

        self._scale_bar_text.set_bbox(
            dict(
    ...
    def _updateScaleBarLocation(self):
        ...
        if self.ui.checkBox_text_follow_bar.isChecked():
            ...
            self._scale_bar_text.set_x(self.ui.doubleSpinBox_bar_location_j.value())
            self._scale_bar_text.set_y(self.ui.doubleSpinBox_bar_location_i.value())
            self._scale_bar_text.set_rotation(
    ...
    def _chooseRoundLength(
        self, pixel_length: float, max_pixels: float, min_pixels: float
    ) -> float:
        """
    ...
    def saveScaleBarMeta(self):
        ...
        self._setMeta(
            "length_per_pixel", self.ui.doubleSpinBox_length_per_pixel.value()
        )
        ...
        self._setMeta(
            "text_va", self.ui.comboBox_text_va.currentText()
        )  # Vertical alignment
        self._setMeta(
            "text_ha", self.ui.comboBox_text_ha.currentText()
        )  # Horizontal alignment
        self._setMeta("text_rotation", self.ui.spinBox_text_rotation.value())
        self._setMeta(
            "text_font_family", self.ui.comboBox_text_font_family.currentText()
        )
        ...
        self._setMeta(
            "text_bg_fill_visible", self.ui.checkBox_text_bg_fill_visible.isChecked()
        )
        self._setMeta(
            "text_bg_fill_alpha", self.ui.doubleSpinBox_text_bg_fill_alpha.value()
        )
        self._setMeta(
            "text_bg_edge_width", self.ui.doubleSpinBox_text_bg_edge_width.value()
        )
        self._setMeta(
            "text_bg_fill_color", self.ui.comboBox_text_bg_fill_color.currentText()
        )
        self._setMeta(
            "text_bg_edge_color", self.ui.comboBox_text_bg_edge_color.currentText()
        )
        ...
        def safe_set_ui(control, value, value_type=None):
            ...
            try:
                ...
                if value_type:
                    value = value_type(
                        value
                    )  # Convert the value if a type is specified
