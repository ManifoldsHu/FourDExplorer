...
class WidgetAlignmentManual(QWidget):
    ...
    def _initTableWidget(self):
        ...
        self.ui.tableWidget_manual_sample.setHorizontalHeaderLabels(
            ["location", "shift"]
        )
        self.ui.tableWidget_manual_sample.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        ...
        self.ui.groupBox_sample_records.layout().insertWidget(
            0, self._table_widget_toolbar
        )
    ...
    def _onShowShiftedDPChanged(self):
        ...
        """
        self.ui.label_measured_shift.setText(
            f"({self.current_shift_i}, {self.current_shift_j})"
        )
    ...
    def _onDownButtonClicked(self):
        ...
        """
        self.ui.doubleSpinBox_manual_shift_i.setValue(
            self.ui.doubleSpinBox_manual_shift_i.value() + 1
        )
    ...
    def _onLeftButtonClicked(self):
        ...
        """
        self.ui.doubleSpinBox_manual_shift_j.setValue(
            self.ui.doubleSpinBox_manual_shift_j.value() - 1
        )
        ...
        if result == QDialog.Accepted:
            if _dialog_generate_shift_map.radio_button_apply_current_shift_vec.isChecked():
                ...
                try:
                    ...
                    QMessageBox.warning(
                        self, "Error", "There must be at least 3 anchors!"
                    )
                    return
            elif (
                _dialog_generate_shift_map.radio_button_quadratic_polynomial.isChecked()
            ):
                try:
                    ...
                    self.logger.error(f"{e}")
                    QMessageBox.warning(
                        self, "Error", "There must be at least 6 anchors!"
                    )
                    return
        else:
            return
        ...
        if result == QDialog.Accepted:
            ...
            try:
                self.hdf_handler.addNewData(
                    group_path, item_name, shape=(2, scan_i, scan_j)
                )
            ...
            if _dialog_generate_shift_map.radio_button_apply_current_shift_vec.isChecked():
                shift_map_dataset.attrs["Alignment/InterpolationMethod"] = (
                    "Shift Vector Broadcast"
                )
            elif _dialog_generate_shift_map.radio_button_linear_regression.isChecked():
                shift_map_dataset.attrs["Alignment/InterpolationMethod"] = (
                    "Linear Regression"
    ...
    def _useLinearRegression(self):
        ...
        i_grid, j_grid = np.meshgrid(
            np.arange(scan_i), np.arange(scan_j), indexing="ij"
        )
    ...
    def _getAnchorLists(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(table_widget.rowCount()):
            try:
                ...
                self.logger.warning(
                    f"Invalid string format at row {i + 1} in the table widget."
                )
                QMessageBox.warning(
                    self,
                    "Invalid Entry",
                    f"Invalid string format at row {i + 1}. Please check the format.",
                )
                ...
                QMessageBox.warning(
                    self,
                    "Missing Entry",
                    f"Missing item at row {i + 1}. Please ensure all cells are filled.",
                )
        ...
        for row in range(self.table_widget.rowCount()):
            ...
            if location_item and location_item.text() == current_location_str:
                if shift_item and shift_item.text() != current_shift_str:
                    ...
                    msgbox.setText(
                        f"The location {current_location_str} already exists with a different shift. Do you want to update it?"
                    )
                    ...
                    if result == QMessageBox.Yes:
                        self.table_widget.setItem(
                            row, 1, QTableWidgetItem(current_shift_str)
                        )
                return

        self.table_widget.insertRow(self.table_widget.rowCount())
        self.table_widget.setItem(
            self.table_widget.rowCount() - 1, 0, QTableWidgetItem(current_location_str)
        )
        self.table_widget.setItem(
            self.table_widget.rowCount() - 1, 1, QTableWidgetItem(current_shift_str)
        )
...
class ActionModifyAnchor(ActionShiftAnchorBase):
    ...
    def modifyItem(self):
        ...
        if current_index < 0:
            return
        modify_dialog = ModifyAnchorDialog(
            parent=None, table_widget=self.table_widget, current_index=current_index
        )
        ...
        if result == QDialog.Accepted:
            ...
            self.table_widget.setItem(
                current_index, 1, QTableWidgetItem(str((shift_i, shift_j)))
            )
...
class ActionImportAnchor(ActionShiftAnchorBase):
    ...
    def _validateDataset(self, dataset: Dataset):
        ...
        if "Alignment/ManualMethod" not in dataset.attrs:
            raise ValueError(
                "The selected dataset does not contain the required attributes."
            )
    ...
    def _parseAnchors(self, dataset: Dataset) -> tuple[list, list]:
        ...
        """
        anchor_locations = [
            strToTuple(loc) for loc in dataset.attrs["Alignment/ManualMethod"]
        ]
        anchor_shifts = [
            strToTuple(shift) for shift in dataset.attrs["Alignment/ManualMethod"]
        ]
        return anchor_locations, anchor_shifts
    ...
    def _handleExistingRecords(self, anchor_locations: list, anchor_shifts: list):
        ...
        if self.table_widget.rowCount() > 0:
            ...
            self._processImport(
                anchor_locations, anchor_shifts, 0
    ...
    def _showConfirmationDialog(self) -> int:
        ...
        msg_box.setText(
            "The table already contains some records. How would you like to proceed?"
        )
    ...
    def _appendNewRecord(self, loc: tuple, shift: tuple):
        ...
        self.table_widget.insertRow(self.table_widget.rowCount())
        self.table_widget.setItem(
            self.table_widget.rowCount() - 1, 0, QTableWidgetItem(str(loc))
        )
        self.table_widget.setItem(
            self.table_widget.rowCount() - 1, 1, QTableWidgetItem(str(shift))
        )
...
class ModifyAnchorDialog(QDialog):
    ...
    def initUi(self):
        ...
        location_label = QLabel(
            f"Location: {self._table_widget.item(self._current_index, 0).text()}"
        )
        layout.addWidget(location_label)

        shift_i_label = QLabel("Shift i:")
        shift_i_input = QSpinBox()
        shift_i_input.setValue(
            strToTuple(self._table_widget.item(self._current_index, 1).text())[0]
        )
        ...
        shift_j_input.setValue(
            strToTuple(self._table_widget.item(self._current_index, 1).text())[1]
        )
...
class GenerateShiftVectorDialog(QDialog):
    ...
    def initUi(self):
        ...
        radio_button_apply_current_shift_vec = QRadioButton(
            "Apply the current shift vector to all the diffraction patterns."
        )
        radio_button_linear_regression = QRadioButton("Linear Regression")
        radio_button_quadratic_polynomial = QRadioButton(
            "Quadratic Polynomial Regression"
        )
