...
class PageViewFourDSTEM(PageBaseFourDSTEM):
    ...
    def _initUi(self):
        ...
        Initialize ui.
        """
        self.ui.pushButton_browse_preview.clicked.connect(self._browsePreview)
    ...
    def setPreview(self, preview_path: str):
        ...
        if not isinstance(preview_path, str):
            raise TypeError(
                "preview_path must be a str, not {0}".format(
                    type(preview_path).__name__
                )
            )
        ...
        if preview_data_obj.shape[0] != scan_i or preview_data_obj.shape[1] != scan_j:
            raise ValueError(
                "The preview image's shape must be equal to the"
        ...
        self._preview_scale_bar_dialog.setScaleBar(
            self._preview_scale_bar, self._preview_scale_bar_text
        )
        self._preview_scale_bar_dialog.setBlitManager(self.preview_blit_manager)
        self._preview_scale_bar_dialog.setPixelLengthMeta(
            "/Calibration/Space/pixel_size_j"
        )
    ...
    def _createPreviewCursor(self):
        ...
        if self.preview_hcursor_object is None:
            ...
            self.preview_blit_manager["preview_hcursor"] = (
                self.preview_hcursor_object
            )  # Here is not a tuple
        else:
            self.preview_hcursor_object.set_ydata(self.scan_ii)
        ...
        if self.preview_vcursor_object is None:
            ...
            self.preview_blit_manager["preview_vcursor"] = (
                self.preview_vcursor_object
            )  # Here is not a tuple

        else:
            self.preview_vcursor_object.set_xdata(self.scan_jj)
            self.preview_blit_manager.update()

        if self.preview_rcursor_object is None:
            self._preview_rcursor_object = self.preview_ax.add_patch(
                Annulus(
            ...
            self.preview_blit_manager["preview_rcursor"] = (
                self.preview_rcursor_object
            )  # Here is not a tuple
        else:
            self.preview_rcursor_object.set_center(xy=(self.scan_jj, self.scan_ii))
            self.preview_blit_manager.update()
