...
class PageAlignFourDSTEM(PageBaseFourDSTEM):
    ...
    def _initUi(self):
        ...
        self.ui.comboBox_show_alignment_method.currentIndexChanged.connect(
            self._changeAlignmentMethod
        )

        self._doubleSpinBox_circle_center_i = (
            self.ui.widget_auxiliary_circle.ui.doubleSpinBox_circle_center_i
        )
        self._doubleSpinBox_circle_center_j = (
            self.ui.widget_auxiliary_circle.ui.doubleSpinBox_circle_center_j
        )
        self._doubleSpinBox_circle_radius = (
            self.ui.widget_auxiliary_circle.ui.doubleSpinBox_circle_radius
        )
        ...
        self.ui.checkBox_set_auxiliary_circle_center_to_shift.stateChanged.connect(
            self._toggleAuxiliaryCircleCenterMode
        )

        self.ui.pushButton_browse_shift_mapping.clicked.connect(
            self._browseShiftMapping
        )
    ...
    def _createFDDNetEllipse(self):
        ...
        self.ui.stackedWidget_align_mode_fddnet.setEllipsePatch(
            self._fddnet_ellipse_object
        )
    ...
    def _browseShiftMapping(self):
        ...
        if not isinstance(shift_mapping, Dataset):
            self.logger.error(f"The selected path is not a dataset: {current_path}")
            QMessageBox.warning(
                self, "Invalid Path", "The selected path is not a dataset."
            )
            return
        ...
        if shift_mapping.shape != (2, scan_i, scan_j):
            ...
            QMessageBox.warning(
                self,
                "Invalid Path",
                "The shape of the selected shift mapping should be matched to (2, scan_i, scan_j), but got: "
                + str(shift_mapping.shape),
            )
            return
    ...
    def startCalculation(self):
        ...
        if not self.ui.lineEdit_shift_mapping_path.text():
            self.logger.error(
                "Cannot shift diffraction patterns without a valid shift mapping."
            )
            QMessageBox.warning(
                self,
                "Invalid Path",
        ...
        if not self.ui.lineEdit_data_path.text():
            ...
            QMessageBox.warning(
                self, "Invalid Path", "Please select a valid 4D-STEM data path."
            )
    ...
    def _generateShiftedMeta(self, output_name: str) -> dict:
        ...
        meta["/Calibration/DiffractionAlignment/shift_mapping_path"] = (
            self.ui.lineEdit_shift_mapping_path.text()
        )
        ...
        meta["/General/title"] = (
            output_name if output_name else "Aligned 4D-STEM Dataset"
        )
