...
4D-Explorer 软件默认使用 HDF5 进行四维数据及其元数据的管理。对于其他格式的文件，例如由
EMPAD 所产生的数据，4D-Explorer 将其转换为hdf5进行管理。出于对内存空间资源节约的考虑，
所有有关四维数据的计算都是基于硬盘 IO 的，而在代码层面上不会将数据一次性全部读入内存中
(尽管操作系统会尝试这样做，但即使是内存空间不足，也无需担心会对性能造成影响。)

这样，对于由仪器采集而来的数据，使用 4D-Explorer 软件进行数据读取的步骤就是：
...
Perform read and write operations on HDF5 files.

4D-Explorer software uses HDF5 by default for managing four-dimensional data and
its metadata. For files in other formats, such as those generated by EMPAD,
4D-Explorer converts them to HDF5 for management. Considering the conservation
of memory resources, all calculations related to four-dimensional data are based
on disk I/O, and the data is not read into memory all at once at the code level
(although the operating system may attempt to do so, there is no need to worry
about performance impact even if memory space is insufficient).

Thus, for data collected by instruments, the steps for reading data using 4D-
Explorer software are:
    - Create a temporary HDF5 data file in a folder, the main part of which is a copy of the original four-dimensional data;
    - Perform memory mapping based on the characteristics of HDF5;
    - Correct and analyze, with results stored in the HDF5 file;
    - Output analysis results;
    - Save or delete the HDF5 file.

In this workflow, the time cost of copying the four-dimensional data is significant.
To save time for subsequent analysis, the HDF5 file can be saved. When reopening
the dataset later, memory mapping can be established directly, saving the time
overhead of loading the four-dimensional data. When reopening, the code in
HDFManager.py will be directly invoked.
...
class HDFHandler(QObject):
    """
    HDFManager 是一个负责管理 4D-Explorer 应用程序中 HDF5 文件的类。它处理 HDF5 文
    件的打开、关闭和操作，并提供文件树结构的模型。

    在 4D-Explorer 中，文件扩展名用于识别 HDF5 文件中数据集的类型和用途。这些扩展名帮
    助应用程序确定如何处理和显示数据。支持的扩展名包括：

    - .4dstem: 此扩展名用于 4D-STEM 数据集，这些数据集是四维数组，表示在二维扫描中每个
    点的衍射图案。

    - .img: 此扩展名用于灰度图像，这些图像是二维数组，表示强度值。

    - .vec: 此扩展名用于二维矢量场，这些矢量场由两个分量组成，每个分量各自是一张灰度图
    像，分别表示矢量场的 i 和 j 分量。

    - .line: 此扩展名用于一维数据集，这些数据集表示单行数据，通常用于光谱或其他线性数据。

    这些扩展名对于应用程序正确解释和处理数据至关重要，确保应用适当的处理和可视化方法。

    HDFManager is a class responsible for managing HDF5 files within the
    4D-Explorer application. It handles the opening, closing, and manipulation
    of HDF5 files, and provides a model for the file tree structure.

    In 4D-Explorer, file extensions are used to identify the type and purpose of
    datasets within HDF5 files. These extensions help the application determine
    how to process and display the data. The supported extensions are:

    - .4dstem: This extension is used for 4D-STEM datasets, which are four-
    dimensional arrays representing diffraction patterns collected at each point
    in a two-dimensional scan.

    - .img: This extension is used for grayscale images, which are two-dimensional
    arrays representing intensity values.

    - .vec: This extension is used for two-dimensional vector fields, which
    consist of two components, each being a grayscale image representing the i
    and j components of the vector field.

    - .line: This extension is used for one-dimensional datasets, which represent
    a single line of data, often used for spectra or other linear data.

    These extensions are crucial for the application to correctly interpret and
    handle the data, ensuring that the appropriate processing and visualization
    methods are applied.

    Attributes:
        file_path (str): The path to the currently opened HDF5 file.

        file (h5py.File): The currently opened HDF5 file object, or None if no
            file is opened.

        model (HDFTreeModel): The model representing the file tree structure.

    Signals:
        file_about_to_close: Emitted just before the file is about to be closed.

        file_closed: Emitted after the file has been closed.

        file_opened: Emitted after a new file has been successfully opened.

        model_created: Emitted after the model has been created.
    """
    ...
    def _initializeFile(self, file: h5py.File):
        ...
        root.attrs["/General/time"] = datetime.now().strftime("%H:%M:%S")
        root.attrs["/General/fourd_explorer_version"] = ".".join(
            str(v) for v in APP_VERSION
        )
    ...
    def addChildDeepFirst(self, parent: "HDFGroupNode"):
        ...
        if not isinstance(parent, HDFGroupNode):
            raise TypeError(
                "parent must be an HDFGroupNode, not {0}".format(type(parent).__name__)
            )
    ...
    def getNode(self, hdf_path: str) -> "HDFTreeNode":
        ...
        if not isinstance(hdf_path, str):
            raise TypeError(
                ("hdf_path must be str, not {0}".format(type(hdf_path).__name__))
            )
        elif hdf_path == "":
            ...
            for key in keys_array:
                if key in node:
                    ...
                    raise KeyError(
                        ("There is no key {0} in {1}".format(key, node.name))
                    )
            return node
    ...
    def addNewGroup(self, parent_path: str, name: str):
        ...
        if not isinstance(parent_path, str):
            raise TypeError(
                ("parent_path must be str, not {0}".format(type(parent_path).__name__))
            )
        elif not isinstance(name, str):
            raise TypeError(("name must be str, not {0}".format(type(name).__name__)))
        parent_node = self.getNode(parent_path)
        if name in parent_node:
            raise ValueError(
                (
                    "name {0} exists in {1}\npath: {2}".format(
                        name, parent_node.name, parent_path
                    )
                )
            )
    ...
    def deleteItem(self, item_path: str):
        ...
        if not isinstance(item_path, str):
            raise TypeError(
                ("hdf_path must be a str, not {0}".format(type(item_path).__name__))
            )
        elif item_path == "/":
    ...
    def moveItem(self, item_path: str, dest_parent_path: str):
        ...
        if not isinstance(item_path, str):
            ...
            raise TypeError(
                (
                    "dest_parent_path must be a str, not {0}".format(
                        type(dest_parent_path).__name__
                    )
                )
            )
        ...
        if isinstance(this_node, HDFRootNode):
            ...
            raise ValueError(
                ("Destination path must be a group: {0}".format(dest_parent_path))
            )
            ...
            raise ValueError(
                (
                    "Name {0} exists in destination: {1}".format(
                        this_node.name, dest_parent_path
                    )
                )
            )
        ...
        this_parent_model_index = self.model.indexFromPath(this_parent_node.path)
        ...
        self.logger.debug("Move {0} to {1}".format(item_path, dest_parent_path))
    ...
    def copyItem(self, item_path: str, dest_parent_path: str):
        ...
        self.logger.debug(
            "Copy task added, from {0} to {1}".format(item_path, dest_parent_path)
        )
    ...
    def changeDataType(self, item_path: str, new_type: HDFType):
        ...
        if not isinstance(item_path, str):
            ...
            raise TypeError(
                (
                    "new_type must be a member of HDFType, not {0}",
                    format(type(new_type).__name__),
                )
            )
    ...
    def matchNodeGenerator(self, kw: str):
        ...
        if not isinstance(kw, str):
            raise TypeError(("kw must be a str, not {0}".format(type(kw).__name__)))
...
class HDFTreeNode(Mapping):
    ...
    def name(self, new_name: str):
        ...
        if new_name == "":
            ...
            if not isinstance(new_name, str):
                raise TypeError(
                    ("new_name must be a str, not {0}".format(type(new_name)))
                )
    ...
    def parent(self, parent: "HDFGroupNode"):
        ...
        if parent is None:
            ...
            raise TypeError(
                "parent must be HDFTreeNode or NoneType, not {0}".format(type(parent))
            )
        elif parent is self:
    ...
    def isSubNode(self, node: "HDFTreeNode") -> bool:
        ...
        if not isinstance(node, HDFTreeNode):
            raise TypeError(
                ("node must be an HDFTreeNode, not {0}".format(type(node).__name__))
            )
...
class HDFGroupNode(HDFTreeNode):
    ...
    def __setitem__(self, key: str, child: HDFTreeNode):
        ...
        if not isinstance(key, str):
            raise TypeError(
                ("Key must be a string, not {0}".format(type(key).__name__))
            )
        if not isinstance(child, HDFTreeNode):
            raise TypeError(
                ("child must be a HDFTreeNode, not {0}".format(type(child).__name__))
            )
    ...
    def addChild(self, child: HDFTreeNode):
        ...
        if child.name in self:
            raise ValueError(("Child name already exists: {0}".format(child.name)))
        else:
            self._mapping[child.name] = child
            child.parent = self
    ...
    def deleteChild(self, child: HDFTreeNode) -> HDFTreeNode:
        ...
        if not isinstance(child, HDFTreeNode):
            raise TypeError(
                ("child must be an HDFTreeNode, not {0}".format(type(child).__name__))
            )
        elif not child.name in self:
            raise KeyError(
                ("child name not found in this node: {0}".format(child.name))
            )
...
class HDFDataNode(HDFTreeNode):
    ...
    def name(self, new_name: str):
        ...
        if not isinstance(new_name, str):
            raise TypeError(
                ("new_name must be a str, not {0}".format(type(new_name).__name__))
            )
        elif new_name == "":
            raise ValueError("HDFDataNode name attribute cannot set as null string")
        elif reValidHDFName.fullmatch(new_name) is None:
            raise ValueError("Invalid name: {0}".format(new_name))
        elif new_name[-1] == ".":
            raise ValueError("Invalid name: {0}".format(new_name))
    ...
    def hdf_type(self, new_type: HDFType):
        ...
        if not new_type in Datatypes:
            raise TypeError(
                (
                    "new_type must be also a kind of data,"
                    " i.e. one of these:\n {0}".format(Datatypes)
                )
            )
        self._hdf_type = new_type
...
class HDFTreeModel(QAbstractItemModel):
    ...
    def moveItem(self, child: QModelIndex, dest_parent: QModelIndex):
        ...
        self.hdf_handler.moveItem(
            child_path, dest_parent.data(role=self.DataRoles.PathRole)
        )
        # self.endMoveRows()
...
class TaskCopy(Task):
    ...
    def __init__(
        self, parent: QObject, item_path: str, dest_parent_path: str, new_name: str
    ):
        """
        arguments:
    ...
    def copyItem(
        ...
        if isinstance(src_item, h5py.Group):
            ...
            # Copy data in slices
            self.copyDatasetInSlices(
                src_dset, dest_dset, progress_signal=progress_signal
            )
        else:
    ...
    def copyDatasetInSlices(
        ...
        if total_size <= max_chunk_bytes:
            ...
            total_chunks = np.prod(
                [(dim + size - 1) // size for dim, size in zip(shape, chunk_sizes)],
                dtype=np.int64,
            )
            # Generate slices
...
class HDFAttrModel(QAbstractTableModel):
    ...
    def createItem(self, key: str, value):
        ...
        self.meta[key] = value
        self.endInsertRows()
        self.logger.debug("Create Attribute {0} in {1}".format(key, self.item_path))
    ...
    def deleteItem(self, index: QModelIndex):
        ...
        if not isinstance(index, QModelIndex):
            raise TypeError(
                "index must be QModelIndex, not {0}".format(type(index).__name__)
            )
        if not index.isValid():
            return None
        ...
        del self.meta[key]
        self.endRemoveRows()
        self.logger.debug("Delete Attribute {0} in {1}".format(key, self.item_path))
    ...
    def modifyValue(self, index: QModelIndex, value):
        ...
        self.logger.debug(
            "Change the value of attribute {0} in {1}".format(key, self.item_path)
        )
